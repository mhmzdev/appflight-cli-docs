...
<Tabs groupId="framework">
  ...
  <TabItem label="React Native" value="react-native">
    ...
    Version is read from `android/app/build.gradle` by default:

    ```groovy
    versionName "1.2.3"
    versionCode 45      // → version "1.2.3+45"
    ```

    The CLI parses both fields and combines them as `versionName+versionCode`. Override with flags:
...
|------|-------------------|--------------|
| Economy (free) | 5 (individual) · 10 (org) | Upload blocked — CLI exits `8` |
| First Class | 15 (individual) · 20 (org) | **Rollover** — oldest build auto-rotates, upload succeeds |

### Economy — blocked at cap
...
```
✓ v1.2.3 of com.your.app uploaded successfully.
Rolled over oldest build (v1.0.0) to stay at your plan cap.
```

Version uniqueness is still enforced — rollover never bypasses the duplicate-version check (exit code `5`). Rollover runs best-effort on the underlying storage blob: if the Storage delete fails, the Firestore record is still updated and a weekly sweep cleans up any orphans.
