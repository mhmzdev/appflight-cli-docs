...
✗ Bump the version or use --version.
```
...
<Tabs groupId="framework">
  ...
  <TabItem label="React Native" value="react-native">
    ...
    Bump `versionName` / `versionCode` in `android/app/build.gradle` and rebuild, or override:
...
<Tabs groupId="framework">
  ...
  <TabItem label="React Native" value="react-native">
    ...
    ```
    ✗ APK not found at android/app/build/outputs/apk/stage/release/app-stage-release.apk
    ✗ Did you forget to run `cd android && ./gradlew assembleStageRelease`?
    ```
    ...
    **Fix:**
    ...
    If Gradle outputs to a different path, edit `apkPath` in `appflight.json` manually.
...
| Channel | Use for |
|---|---|
| [GitHub Issues](https://github.com/mhmzdev/appflight-cli-docs/issues) | Bug reports, unexpected CLI behaviour, doc corrections |
| [hamza.6.shakeel@gmail.com](mailto:hamza.6.shakeel@gmail.com) | Account issues, private questions, anything else |
